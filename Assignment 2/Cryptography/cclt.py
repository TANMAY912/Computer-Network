    ...
    def run(self):
        ...
        if in_data.decode() == "REGISTERED TOSEND " + username + "\n":
            ...
            while True:
                ...
                self.csock.sendall(bytes(recvr_name, 'UTF-8'))
                in_data = self.csock.recv(1024)

                if in_data.decode() != "RECVR NOT FOUND\n":
                    pk = in_data
                    print(pk)
...
class RecieveThread(threading.Thread):
    ...
    def run(self):
        ...
        if in_datar.decode() == "REGISTERED TORECV " + username + "\n":
            ...
            pk = bytes("pk",'UTF-8')
            self.rsock.sendall(pk)
            ack_pk = self.rsock.recv(1024)

            if ack_pk.decode() == "PUBLICKEY RECIEVED\n":
                ...
                print("Public key not registered")
                print(ack_pk.decode())
                self.rsock.close()
                return
...
class RecieveThread(threading.Thread):
    ...
    def __init__(self, recievesocket, username):
        threading.Thread.__init__(self)
        print("called")
        self.rsock = recievesocket
        self.username = username
        print("username -> ", self.username)
    ...
    def run(self):
        print("Send message : REGISTER TORECV ")
        self.rsock.sendall(bytes("REGISTER TORECV " + username + "\n\n",'UTF-8'))
        in_data =  self.rsock.recv(1024)
        if in_data.decode() == "REGISTERED TORECV " + username + "\n":
            print("Recieve connection established")
            ...
            while True:
                ...
                if msg.split('\n', 3)[0][:8] != 'FORWARD ' or msg.split('\n', 3)[1][:16] != 'Content-length: ' or msg.split('\n', 3)[2] != '':
                    self.rsock.sendall(bytes('ERROR 103 Header incomplete\n\n', 'UTF-8'))
                    continue

                message = msg.split('\n', 3)[3]
                usrnme = msg.split('\n', 3)[0][8:]
                print("From ", usrnme, ": ",message)


                self.rsock.sendall(bytes('RECEIVED ' + usrnme + '\n\n','UTF-8'))
                # in_data = self.rsock.recv(1024)
                # print("From Server :" ,in_data.decode())
                # if out_data=='bye':
                    # break
